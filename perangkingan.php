...
<div class="container mt-5">
    ...
    if (isset($_POST['calculate'])) {
        echo "<h3 class='text-center mt-4'>Hasil Perhitungan FAHP</h3>";
        echo "<div class='table-responsive'>";
        echo "<table class='table table-bordered table-sm'>";
        echo "<thead class='thead-light'><tr><th>ID</th><th>Nama</th><th>Kamar</th><th>Nilai</th><th>Kelas</th></tr></thead><tbody>";
    
        $hasilFahp = []; // Array untuk menyimpan hasil FAHP
        foreach ($_POST['nilai_kriteria'] as $id_alternatif => $kriteria) {
            ...
            if ($altInfo) {
                $hasilFahp[$id_alternatif] = $kelas; // Simpan kelas FAHP untuk akurasi
                echo "<tr>";
        ...
        // Perhitungan manual
        echo "<h3 class='text-center mt-4'>Perhitungan Manual</h3>";
        echo "<div class='table-responsive'>";
        echo "<table class='table table-bordered table-sm'>";
        echo "<thead class='thead-light'><tr><th>ID</th><th>Nama</th><th>Kamar</th><th>Nilai</th><th>Kelas</th></tr></thead><tbody>";
    
        $hasilManual = []; // Array untuk menyimpan hasil manual
        foreach ($_POST['nilai_kriteria'] as $id_alternatif => $kriteria) {
            $totalManual = array_sum($kriteria);
            $jumlahKriteria = count($kriteria);
            $averageScore = $totalManual / $jumlahKriteria;
    
            // Menentukan kelas berdasarkan rata-rata nilai
            $kelasManual = '';
            if ($averageScore >= 91) {
                $kelasManual = 'MUSTAHIQ';
            ...
            if ($altInfo) {
                $hasilManual[$id_alternatif] = $kelasManual; // Simpan kelas manual untuk akurasi
                echo "<tr>";
                echo "<td>{$altInfo['id_alternatif']}</td>";
                echo "<td>{$altInfo['nama_alternatif']}</td>";
                echo "<td>{$altInfo['kamar']}</td>";
                echo "<td>" . number_format($averageScore, 2) . "</td>";
                echo "<td>{$kelasManual}</td>";
                echo "</tr>";
            } else {
                echo "<tr><td colspan='5'>Data tidak ditemukan untuk ID: $id_alternatif</td></tr>";
            }
        ...
        // Hitung akurasi
        $totalData = count($hasilFahp);
        $matched = 0;
        foreach ($hasilFahp as $id => $kelasFahp) {
            if (isset($hasilManual[$id]) && $hasilManual[$id] === $kelasFahp) {
                $matched++;
            }
        }
        $accuracy = ($matched / $totalData) * 100;
    
        // Tampilkan akurasi
        echo "<h3 class='text-center mt-4'>Hasil Akurasi</h3>";
        echo "<div class='card mt-3 mx-auto' style='max-width: 600px;'>";
        echo "  <div class='card-body'>";
        echo "    <p class='text-center'>Berikut adalah tingkat kecocokan antara kelas yang dihitung menggunakan FAHP dan metode manual:</p>";
        echo "    <table class='table table-borderless text-center'>";
        echo "      <tr><th>Total Data</th><td>{$totalData}</td></tr>";
        echo "      <tr><th>Jumlah Cocok</th><td>{$matched}</td></tr>";
        echo "      <tr><th>Akurasi</th><td><strong>" . number_format($accuracy, 2) . "%</strong></td></tr>";
        echo "    </table>";
        echo "  </div>";
        echo "</div>";
